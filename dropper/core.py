...
from .utils import string_to_hex, io_to_tokens
...
class Dropper:
    ...
    def __init__(
        ...
        self.junk_strings: List[int] = []
        self.callable_map: Dict[str, str] = {}
        ...
        table.add_row(
            "chr(...)", f"{self._chr}(...)", "Translate a character to its ASCII code"
        )

        table.add_row("eval(...)", f"{self._eval}(...)", "Evaluate a Python expression")
        table.add_row("bool(...)", f"{self._bool}(...)", "Convert a value to a boolean")

        table.add_row(
            "bytes(...)", f"{self._bytes}(...)", "Convert a string to a bytes object"
        )
            ...
            if _type == NAME:
                ...
                if string in ("def", "class") and self.obfuscate_names:
                    ...
                    if change_callable_name:
                        self.callable_map[token.string] = (name := self.junk_string())
...
# g3t-dr0pp!d, keybase.io/mclareeen
        """
...
# pylint: ignore=line-too-long\n"""
    ...
    def __obfuscate(self) -> None:
        """Obfuscate the code"""
        self.console.log("Obfuscating objects... (functions, classes, strings, ints)")
        ...
        if self.obfuscate_names:
            ...
            for token in io_to_tokens(BytesIO(obfuscated), False):
                ...
                if _type == NAME and (value := self.callable_map.get(string)):
                    string = value

                final_tokens.append(
                    TokenInfo(_type, string, token.start, token.end, token.line)
                )
        ...
        for name, new_name in self.callable_map.items():
